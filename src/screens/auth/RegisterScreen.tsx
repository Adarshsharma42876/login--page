...
const SignUpScreen = ({ navigation }: any) => {
  ...
  const handlePasswordChange = (text: string) => {
    if (text.length > 7) return; // Prevent input if more than 7 characters
    setPassword(text);
    if (!validatePassword(text)) {
      setPasswordError(
        "Password must be exactly 7 characters, include an uppercase letter, a number, and a special character."
      );
    } else {
      setPasswordError("");
    }
